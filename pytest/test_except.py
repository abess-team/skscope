...
def test_group(model, solver_creator):
    ...
    with pytest.raises(ValueError, match=re.escape("The length of group should be equal to dimensionality.")):
        solver.solve(model["loss"], jit=True)
...
def test_ic(model, solver_creator):
    ...
    with pytest.raises(ValueError, match=re.escape("ic_type should be one of ['aic', 'bic', 'sic','ebic'].")):
        solver.solve(model["loss"], jit=True)
...
def test_cv(model, solver_creator):
    ...
    with pytest.raises(ValueError, match=re.escape("cv_fold_id should be an 1D array of integers.")):
        solver.solve(model["loss"], jit=True)
    solver.set_config(cv_fold_id=np.zeros(1+model["n_samples"]))
    with pytest.raises(ValueError, match=re.escape("The length of cv_fold_id should be equal to sample_size.")):
        solver.solve(model["loss"], jit=True)
    solver.set_config(cv_fold_id=np.zeros(model["n_samples"]))
    with pytest.raises(ValueError, match=re.escape("The number of different elements in cv_fold_id should be equal to cv.")):
        solver.solve(model["loss"], jit=True)
...
def test_init(model, solver_creator):
    solver = solver_creator(model["n_features"], model["n_informative"], model["n_samples"])
    with pytest.raises(ValueError, match=re.escape("The initial active set should be an 1D array of integers.")):
    ...
    with pytest.raises(ValueError, match=re.escape("The length of init_params should be equal to dimensionality.")):
...
def test_log(model):
    ...
    with pytest.raises(ValueError, match=re.escape("console_log_level and file_log_level must be in 'off', 'error', 'warning', 'debug'")):
        solver.solve(model["loss"], jit=True)
...
def test_gs(model):
    ...
    with pytest.raises(ValueError, match=re.escape("gs_lower_bound and gs_upper_bound should be between 0 and dimensionality.")):
        solver.solve(model["loss"], jit=True)
    solver.set_config(gs_upper_bound = model["n_features"], gs_lower_bound = model["n_features"]+1)
    with pytest.raises(ValueError, match=re.escape("gs_upper_bound should be larger than gs_lower_bound.")):
        solver.solve(model["loss"], jit=True)    
...
def test_screening_size(model):
    ...
    with pytest.raises(ValueError, match=re.escape("screening_size should be between sparsity and dimensionality.")):
        solver.solve(model["loss"], jit=True)
